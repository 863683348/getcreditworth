...
for (const book of qualityBooks) {
  ...
  const titleEscaped = esc(book.title);
  ...
  lines.push(`    title: '${titleEscaped} Audiobook Review: ${book.valueScore >= 9 ? 'An Exceptional Credit Pick' : book.valueScore >= 7 ? 'Solid Credit Value' : 'Worth a Listen'}',`);
  ...
  lines.push(`      <>`);
  lines.push(`        <p>`);
  lines.push(`          <strong>${titleEscaped}</strong> by ${authorEscaped} (narrated by ${narratorEscaped}) is a ${runtimeHrs}-hour ${catEscaped.toLowerCase()}. It earned a <strong>${book.starRating}-star rating</strong> from ${book.reviewCount} listeners and a <strong>Value Score of ${book.valueScore.toFixed(1)}</strong>.`);
  lines.push(`        </p>`);
  ...
  lines.push(`          <a href={"/books/${book.asin}"} className="text-primary hover:underline">${titleEscaped} page</a>{" `);
  lines.push(`          or browse more <a href={"/category/${categorySlug}"} className="text-primary hover:underline">${catEscaped}</a> for credit value comparisons.`);
  lines.push(`        </p>`);
  lines.push(`      </>`);
  lines.push(`    ),`);
  lines.push(`  },`);
...
for (let i = 0; i < top50.length && vsCount < 25; i++) {
  for (let j = i + 1; j < top50.length && vsCount < 25; j++) {
