...
export const en = {
  ...
  aboutPage: {
    ...
    editorialDesc: 'Our rankings are generated by a fixed algorithm, not by opinion or affiliate payouts. Every audiobook is scored with the same formula regardless of author, publisher, or commission rate. We never accept payment to include or promote a title, and affiliate links do not influence a book’s Value Score.',
    methodologyTitle: 'Our Methodology',
    methodologyDesc: 'Value Scores are computed from publicly available data — price, runtime, star rating, and review count — via the formula shown above. We apply a log-scaled review adjustment so a 4.8-star book with 10,000 reviews is not treated identically to one with 12 reviews. Scores are a decision aid, not a substitute for listening to samples. Book data is refreshed regularly from the Amazon Product Advertising API.',
    contactTitle: 'Corrections & Contact',
    contactDesc: 'Found a data error or want to suggest a title? Email us at hello@getcreditworth.com. We review correction requests weekly and update our dataset accordingly.',
