...
const PLAN_INFO: Record<Plan, { credits: number; cost: number; name: string; costPerCredit: number }> = {
  ...
  annual: { credits: 12, cost: 149.5, name: "Premium Plus Annual", costPerCredit: 12.46 },
};
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4 sm:p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base sm:text-lg font-semibold text-text-primary">{t.calculatorPage.yourSubscription}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 accent-primary h-8 cursor-pointer"
                style={{ minHeight: "44px" }}
              />
              <span className="font-mono font-bold text-lg text-text-primary w-8 sm:w-10 text-right">
                {credits}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 sm:py-2 rounded-md border border-border bg-bg-base text-sm text-text-primary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary cursor-pointer min-h-[44px]"
            >
        ...
        {/* Plan Summary */}
        <div className="mt-4 p-3 bg-bg-surface rounded-md text-sm space-y-1">
      ...
      <div className="card p-4 sm:p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base sm:text-lg font-semibold text-text-primary">
            {t.calculatorPage.recommendedBooks
              .replace("{credits}", String(credits))
              .replace("{credits, plural, one {Credit} other {Credits}}", credits === 1 ? "Credit" : "Credits")}
          </h2>
        </div>
            ...
            <div key={book.asin} className="flex items-center gap-3 p-3 bg-bg-surface rounded-md">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-secondary truncate">
                  {formatDuration(book.runtimeMinutes)} &ndash; {formatRating(book.starRating)} stars &ndash; {formatPrice(book.price)}
                </p>
              </div>
      ...
      <div className="card p-4 sm:p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base sm:text-lg font-semibold text-text-primary">{t.calculatorPage.valueAnalysis}</h2>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <div className="p-3 sm:p-4 bg-bg-surface rounded-md">
            <div className="flex items-center gap-1.5 text-text-muted text-xs mb-1">
              <DollarSign className="h-3.5 w-3.5" />
              {t.calculatorPage.totalBookValue}
            </div>
            <div className="font-mono font-bold text-base sm:text-lg text-text-primary">
          ...
          <div className={`p-3 sm:p-4 rounded-md ${savings > 0 ? "bg-success/5" : "bg-warning/5"}`}>
            <div className="flex items-center gap-1.5 text-text-muted text-xs mb-1">
              <TrendingUp className="h-3.5 w-3.5" />
              {t.calculatorPage.yourSavings}
            </div>
            <div className={`font-mono font-bold text-base sm:text-lg ${savings > 0 ? "text-success" : "text-warning"}`}>
              {savings > 0 ? "+" : ""}{formatPrice(savings)}
            </div>
            <div className="text-xs text-text-muted mt-0.5">
              {savingsPercent > 0
                ? t.calculatorPage.savingsPercent.replace("{value}", String(savingsPercent.toFixed(0)))
                : t.calculatorPage.noSavings}
            </div>
          </div>
        </div>
